...
public class WhatsAppRecvDocAdapter extends RecyclerView.Adapter<WhatsAppRecvDocAdapter.viewHolder> {
   ...
   public  void setFileSizeListener(FileSizeListener listener){
    ...
    public WhatsAppRecvDocAdapter(List<Uri> reciveArrayList, FragmentActivity activity, recvDocFragment fragment,String from) {
    ...
    public void SendremoveItem(int position) {
       if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q){
           String ImgUri = whatsappStatusModelList.get(position).toString();



           boolean isFileExists = false;
           Cursor cursor = mContext.getContentResolver().query(Uri.parse(ImgUri), null, null, null, null);
           if (cursor != null && cursor.moveToFirst()) {
...
// If file exists, call the deleteDocument method
           if (isFileExists) {
               ...
               try {
                   isDeleted = DocumentsContract.deleteDocument(mContext.getContentResolver(), Uri.parse(ImgUri));
